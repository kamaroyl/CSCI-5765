...
void outputA(struct msg message) {
    ...
    printf("IS CHECKSUM VALID? %s\n", validateChecksum(&packetToSend)? "TRUE": "FALSE");
...
void timerInterruptA(void) {
    printf("\n\nInterrupt Timer A Fired; time: %f\n", getTime());
...
void inputB(struct pkt packet) {
    printf("\n\nInput B; time: %f\n", getTime());
    printf("Current Expected Sequence Number: %d\n", CurrentSequenceNumberB);
    printPacket(&packet);
    struct pkt response;
    response.seqnum = packet.seqnum;
    emptyData(response.payload);
    printf("Last Packet seen at B: \n");
    printPacket(lastPacketB);
    if (packetEqual(&packet, lastPacketB)) {
        printf("Incoming Packet was duplicate\n");
        response.acknum = lastPacketB->seqnum;
    ...
    if (CurrentSequenceNumberB == packet.seqnum) {
        //Copy packet into last packet
        lastPacketB->acknum = packet.acknum;
        lastPacketB->checksum = packet.checksum;
        lastPacketB->seqnum = packet.seqnum;
        for (int i = 0; i < 20; i++) {
            lastPacketB->payload[i] = packet.payload[i];
        }
        printf("Last Packet B is now: \n");
        printPacket(lastPacketB);
        struct msg message;
...
void initB(void){
    CurrentSequenceNumberB = 1;
    lastPacketB = (struct pkt*) malloc(sizeof (struct pkt));
}
...
void init() {                       /* initialize the simulator */
    ...
    printf("-----  Stop and Wait Network Simulator Version 1.3 -------- \n\n");
#ifdef GO_BACK_N
